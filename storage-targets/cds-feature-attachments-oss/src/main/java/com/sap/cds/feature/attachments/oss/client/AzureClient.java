  ...
  public AzureClient(ServiceBinding binding, ExecutorService executor) {
    ...
    this.blobContainerClient =
        new BlobContainerClientBuilder()
            .endpoint(
                binding.getCredentials().get("container_uri").toString()
                    + "?"
    ...
    return executor.submit(
        () -> {
          BlockBlobClient blockBlobClient =
              this.blobContainerClient.getBlobClient(completeFileName).getBlockBlobClient();
          ...
          try (BlobOutputStream outputStream = blockBlobClient.getBlobOutputStream()) {
            ...
            throw new ObjectStoreServiceException(
                "Failed to upload file to the Azure Object Store", e);
          }
          return null;
        });
  }
    ...
    return executor.submit(
        () -> {
          ...
          try {
            ...
            throw new ObjectStoreServiceException(
                "Failed to delete file from the Azure Object Store", e);
          }
          return null;
        });
  }
