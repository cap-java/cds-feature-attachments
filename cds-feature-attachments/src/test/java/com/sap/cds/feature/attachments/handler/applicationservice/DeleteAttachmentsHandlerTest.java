...
class DeleteAttachmentsHandlerTest {
	...
	void attachmentDataExistsServiceIsCalled() {
		...
		verify(modifyAttachmentEvent).processEvent(any(), eq(entity.getElement(Attachment.CONTENT)), eq(inputStream), eq(data), eq(Map.of("ID", data.getId())), eq(context));
	...
	void attachmentDataExistsAsExpandServiceIsCalled() {
		...
		verify(modifyAttachmentEvent).processEvent(any(Path.class), any(), eq(inputStream), eq(attachment1), eq(Map.of("ID", attachment1.getId())), eq(context));
		verify(modifyAttachmentEvent).processEvent(any(Path.class), any(), eq(inputStream), eq(attachment2), eq(Map.of("ID", attachment2.getId())), eq(context));
