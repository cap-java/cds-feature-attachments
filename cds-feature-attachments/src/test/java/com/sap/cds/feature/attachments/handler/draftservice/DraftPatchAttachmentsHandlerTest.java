...
class DraftPatchAttachmentsHandlerTest {
  ...
  // --- Composition-based Attachment Metadata Tests (persistCompositionAttachmentMetadata) ---

  @Test
  void compositionAttachmentPersistsMetadataWhenMimeTypeAndFileNamePresent() {
    // Target the attachment entity directly to trigger persistCompositionAttachmentMetadata
    var draftAttachmentName = Attachment_.CDS_NAME + DraftUtils.DRAFT_TABLE_POSTFIX;
    getEntityAndMockContext(draftAttachmentName);
    ...
    // Simulate event.processEvent setting contentId, mimeType, and fileName in
    // path.target().values()
    when(event.processEvent(any(), any(), any(), any(), any()))
        .thenAnswer(
            invocation -> {
              attachment.setContentId("cid-composition");
    ...
    // Verify update was called with mimeType and fileName
    ArgumentCaptor<CqnUpdate> updateCaptor = ArgumentCaptor.forClass(CqnUpdate.class);
    verify(persistence).run(updateCaptor.capture());
    CqnUpdate update = updateCaptor.getValue();
    assertThat(update.entries()).isNotEmpty();
    assertThat(update.entries().get(0)).containsEntry("mimeType", "application/pdf");
    assertThat(update.entries().get(0)).containsEntry("fileName", "report.pdf");
  }

  @Test
  void compositionAttachmentPersistsOnlyMimeType() {
    var draftAttachmentName = Attachment_.CDS_NAME + DraftUtils.DRAFT_TABLE_POSTFIX;
    getEntityAndMockContext(draftAttachmentName);
    ...
    when(event.processEvent(any(), any(), any(), any(), any()))
        .thenAnswer(
            invocation -> {
              attachment.setContentId("cid-mime-only");
    ...
    verify(persistence).run(updateCaptor.capture());
    CqnUpdate update = updateCaptor.getValue();
    assertThat(update.entries().get(0)).containsEntry("mimeType", "text/plain");
    assertThat(update.entries().get(0)).doesNotContainKey("fileName");
  }

  @Test
  void compositionAttachmentPersistsOnlyFileName() {
    var draftAttachmentName = Attachment_.CDS_NAME + DraftUtils.DRAFT_TABLE_POSTFIX;
    getEntityAndMockContext(draftAttachmentName);
    ...
    when(event.processEvent(any(), any(), any(), any(), any()))
        .thenAnswer(
            invocation -> {
              attachment.setContentId("cid-filename-only");
    ...
    verify(persistence).run(updateCaptor.capture());
    CqnUpdate update = updateCaptor.getValue();
    assertThat(update.entries().get(0)).containsEntry("fileName", "data.csv");
    assertThat(update.entries().get(0)).doesNotContainKey("mimeType");
  }

  @Test
  void compositionAttachmentSkipsUpdateWhenContentIdNull() {
    var draftAttachmentName = Attachment_.CDS_NAME + DraftUtils.DRAFT_TABLE_POSTFIX;
    getEntityAndMockContext(draftAttachmentName);
    ...
    // processEvent does NOT set contentId
    when(event.processEvent(any(), any(), any(), any(), any())).thenReturn(null);

    cut.processBeforeDraftPatch(eventContext, List.of(attachment));

    // Should only call persistence.run once for the SELECT, not for UPDATE
    verify(persistence).run(any(CqnSelect.class));
    verify(persistence, never()).run(any(CqnUpdate.class));
  }

  @Test
  void compositionAttachmentSkipsUpdateWhenNoMetadata() {
    var draftAttachmentName = Attachment_.CDS_NAME + DraftUtils.DRAFT_TABLE_POSTFIX;
    getEntityAndMockContext(draftAttachmentName);
    ...
    // processEvent sets contentId but no mimeType/fileName
    when(event.processEvent(any(), any(), any(), any(), any()))
        .thenAnswer(
            invocation -> {
              attachment.setContentId("cid-no-metadata");
              return null;
            });
