...
public final class MediaTypeService {
  ...
  /**
   * Resolves the MIME type of a file based on its filename (specifically its
   * extension).
   *
   * @param fileName the name of the file (including extension)
   * @return the resolved MIME type, or a default MIME type if it cannot be
   *         determined
  ...
  /**
   * Checks if a given MIME type is allowed based on a collection of acceptable
   * media
   * 
  ...
  public static boolean isMimeTypeAllowed(
    ...
    Collection<String> normalizedTypes = acceptableMediaTypes.stream().map(type -> type.trim().toLowerCase()).toList();
