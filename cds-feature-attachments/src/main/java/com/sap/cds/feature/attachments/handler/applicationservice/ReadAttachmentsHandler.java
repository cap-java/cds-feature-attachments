 ...
 * attachments for entities. In the before read event, it modifies the CQN to
 * include the content ID
 * and status. In the after read event, it adds a proxy for the stream of the
 * attachments service to
 * the data. Only if the data are read the proxy forwards the request to the
 * attachment service to
 * read the attachment. This is needed to have a filled stream in the data to
 * enable the OData V4
...
public class ReadAttachmentsHandler implements EventHandler {
    ...
    void processAfter(CdsReadEventContext context, List<CdsData> data) {
        if (ApplicationHandlerHelper.containsContentField(context.getTarget(), data)) {
            ...
            // Check if this is a content request by examining the target
            boolean isContentRequest = context.getTarget() != null
                    && "content".equals(context.getTarget().getName());

            logger.debug("Is content request: {}", isContentRequest);
            ...
            Converter converter = (path, element, value) -> {
                ...
                if (nonNull(attachment.getContentId())) {
                    ...
                    Supplier<InputStream> supplier = nonNull(content)
                            ? () -> content
                            : () -> attachmentService.readAttachment(attachment.getContentId());

                    // Add a condition to simulate exception for testing
                    if ("simulate-error.png".equals(attachment.getFileName())) {
                        supplier = () -> {
                            throw new ServiceException("SIMULATED EXCEPTION FOR TESTING");
                        };
                    }
                    try {
                        return new LazyProxyInputStream(supplier, statusValidator, attachment.getStatus(), isContentRequest);
                    } catch (java.io.IOException ex) {
                        // Unwrap ServiceException if present
                        if (ex.getCause() instanceof ServiceException) {
                            throw (ServiceException) ex.getCause();
                        }
                        throw new ServiceException("Failed to initialize attachment stream", ex);
                    }
                } else {
                    return value;
                }
    ...
    private void verifyStatus(Path path, Attachments attachment) {
        if (areKeysEmpty(path.target().keys())) {
            ...
            logger.debug("In verify status for content id {} and status {}", attachment.getContentId(), currentStatus);
            if (StatusCode.UNSCANNED.equals(currentStatus) || StatusCode.SCANNING.equals(currentStatus)
                    || currentStatus == null) {
                logger.debug("Scanning content with ID {} for malware, has current status {}",
                        attachment.getContentId(), currentStatus);
