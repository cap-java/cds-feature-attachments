...
/**
 * The class {@link CreationChangeSetListener} is a listener that is called
 * after the transaction is
 * closed for which the listener was registered. It marks the attachment as
 * deleted by calling the
...
public class CreationChangeSetListener implements ChangeSetListener {
     ...
     *
     * @param contentId         the content ID of the attachment to be marked as
     *                          deleted
     * @param cdsRuntime        the {@link CdsRuntime} to access the request context
     * @param attachmentService the {@link AttachmentService} to mark the attachment
     *                          as deleted
     */
    ...
    public void afterClose(boolean completed) {
        if (!completed) {
            ...
            cdsRuntime
                    .requestContext()
                    .run(
                            (Consumer<RequestContext>) requestContext -> attachmentService.markAttachmentAsDeleted(
