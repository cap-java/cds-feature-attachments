...
public class DraftCancelAttachmentsHandler implements EventHandler {
	...
	private Validator buildDeleteContentValidator(DraftCancelEventContext context, List<CdsData> activeCondensedAttachments) {
	...
	private CdsStructuredType getActiveEntity(DraftCancelEventContext context) {
		return isDraftEntity(context) ? context.getTarget().getTargetOf("SiblingEntity") : context.getTarget();
	}

	private CdsStructuredType getDraftEntity(DraftCancelEventContext context) {
		return isDraftEntity(context) ? context.getTarget() : context.getTarget().getTargetOf("SiblingEntity");
	}
	...
	private List<CdsData> readAttachments(DraftCancelEventContext context, CdsStructuredType entity, boolean isActiveEntity) {
		var cqnInactiveEntity = CQL.copy(context.getCqn(), activeEntityModifierProvider.getModifier(isActiveEntity, entity.getQualifiedName()));
