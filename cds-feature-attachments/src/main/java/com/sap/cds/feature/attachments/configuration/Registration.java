...
public class Registration implements CdsRuntimeConfiguration {
	...
	public void eventHandlers(CdsRuntimeConfigurer configurer) {
		logger.debug("Registering event handler for attachment service");

		// get some required services from service catalog
		...
		// get first service binding for the malware scan service
		...
		var clientProviderFactory = new MalwareScanClientProviderFactory(binding, configurer.getCdsRuntime());
		...
		// register event handlers on ApplicationService events
		...
		EndTransactionMalwareScanRunner malwareScanRunner = new EndTransactionMalwareScanRunner(null, null,
				malwareScanner, configurer.getCdsRuntime());
		configurer.eventHandler(new ReadAttachmentsHandler(attachmentService, BeforeReadItemsModifier::new,
				new DefaultAttachmentStatusValidator(), malwareScanRunner));

		// register event handlers on DraftService events
