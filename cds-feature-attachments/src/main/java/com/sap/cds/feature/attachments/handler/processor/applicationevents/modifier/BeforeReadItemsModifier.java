...
public class BeforeReadItemsModifier implements Modifier {
	...
	private List<CqnSelectListItem> addDocumentIdItem(List<CqnSelectListItem> list) {
		...
		var fieldOptional = getNewFieldForMediaAssociation(ROOT_ASSOCIATION);
	...
	private List<CqnSelectListItem> processExpandedEntities(List<CqnSelectListItem> expandedItems) {
		...
		expandedItems.forEach(item -> {
			...
			var fieldOptional = getNewFieldForMediaAssociation(item.asExpand().displayName());
	...
	private Optional<CqnSelectListItem> getNewFieldForMediaAssociation(String association) {
		return mediaAssociations.contains(association) ? Optional.of(CQL.get(Attachments.DOCUMENT_ID)) : Optional.empty();
	}
