...
public class MockHttpRequestHelper {
  ...
  @Autowired private JsonToCapMapperTestHelper mapper;
  ...
  public MvcResult executeGet(String url) throws Exception {
    MockHttpServletRequestBuilder requestBuilder =
        MockMvcRequestBuilders.get(url).contentType(contentType).accept(accept);
    return mvc.perform(requestBuilder).andReturn();
  }

  public String executeGetWithSingleODataResponseAndAssertStatus(String url, HttpStatus status)
      throws Exception {
  ...
  public <T extends CdsData> T executeGetWithSingleODataResponseAndAssertStatus(
      String url, Class<T> resultType, HttpStatus status) throws Exception {
  ...
  public CdsData executePostWithODataResponseAndAssertStatusCreated(String url, String body)
      throws Exception {
    return executePostWithODataResponseAndAssertStatus(url, body, HttpStatus.CREATED);
  }

  public void executePatchWithODataResponseAndAssertStatusOk(String url, String body)
      throws Exception {
    executePatchWithODataResponseAndAssertStatus(url, body, HttpStatus.OK);
  }

  public CdsData executePostWithODataResponseAndAssertStatus(
      String url, String body, HttpStatus status) throws Exception {
    MvcResult result = executePost(url, body);
    String resultBody = result.getResponse().getContentAsString();
    assertThat(result.getResponse().getStatus())
        .as("Unexpected HTTP status, with response body " + resultBody)
        .isEqualTo(status.value());
    return mapper.mapResponseToSingleResult(resultBody);
  }

  public void executePatchWithODataResponseAndAssertStatus(
      String url, String body, HttpStatus status) throws Exception {
    executePatchWithODataResponseAndAssertStatus(url, body, "*", status);
  }

  public void executePatchWithODataResponseAndAssertStatus(
      String url, String body, String etag, HttpStatus status) throws Exception {
