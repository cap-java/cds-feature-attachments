...
abstract class OdataRequestValidationBase {
  ...
  void directDeleteOfContentClears() throws Exception {
    ...
    executeDeleteAndCheckNoDataCanBeRead(
        buildDirectAttachmentEntityUrl(itemAttachment.getId()),
        itemAttachmentAfterChange.getContentId());
  ...
  void updateContentWorksForUrlsWithNavigation() throws Exception {
    ...
    verifyContentAndContentId(attachment, content, itemAttachment);
    verifySingleCreateAndUpdateEvent(
        attachment.getContentId(), itemAttachment.getContentId(), content);
  }
  ...
  void errorInTransactionAfterCreateCallsDelete() throws Exception {
    ...
    putContentForAttachmentWithNavigation(
        selectedRoot, itemAttachment, status().is5xxServerError());
    var attachment = selectUpdatedAttachmentWithExpand(selectedRoot, itemAttachment);
  ...
  protected Items selectItem(Items item) {
    ...
    return selectedRootAfterContentCreated.getItems().stream()
  ...
  private void verifySelectedRoot(Roots selectedRoot, Roots serviceRoot) {
    ...
    assertThat(selectedRoot.getAttachments())
        .hasSize(1)
        .first()
        .satisfies(
            attachment -> {
    ...
    assertThat(selectedRoot.getItems())
        .hasSize(2)
        .first()
        .satisfies(
            item -> {
    ...
    assertThat(selectedRoot.getItems().get(1).getTitle())
        .isEqualTo(serviceRoot.getItems().get(1).getTitle());
  ...
  protected Items getItemWithAttachment(Roots selectedRoot) {
    return selectedRoot.getItems().stream()
        .filter(item -> !item.getAttachments().isEmpty())
        .findAny()
        .orElseThrow();
  }
  ...
  private Items getItemWithAttachmentEntity(Roots selectedRoot) {
    return selectedRoot.getItems().stream()
        .filter(item -> !item.getAttachmentEntities().isEmpty())
        .findAny()
        .orElseThrow();
  }

  protected String putContentForAttachmentWithNavigation(
      Roots selectedRoot, Attachments itemAttachment) throws Exception {
    return putContentForAttachmentWithNavigation(
        selectedRoot, itemAttachment, status().isNoContent());
  }
  ...
  protected String buildExpandAttachmentUrl(String rootId, String itemId) {
    return "/odata/v4/TestService/Roots("
        ...
        + itemId
        + ")"
        + "?$expand=attachments,attachmentEntities";
  }
  ...
  private String putContentForAttachmentWithoutNavigation(
      AttachmentEntity itemAttachment, ResultMatcher matcher) throws Exception {
    var url = buildDirectAttachmentEntityUrl(itemAttachment.getId()) + "/content";
    var testContent = "testContent" + itemAttachment.getNote();
    requestHelper.setContentType(MediaType.APPLICATION_OCTET_STREAM);
    requestHelper.executePutWithMatcher(url, testContent.getBytes(StandardCharsets.UTF_8), matcher);
  ...
  private Attachments selectUpdatedAttachmentWithExpand(
    ...
    return items.getAttachments().stream()
        .filter(attach -> itemAttachment.getId().equals(attach.getId()))
        .findAny()
        .orElseThrow();
  }
  ...
  private void executeDeleteAndCheckNoDataCanBeRead(String baseUrl, String contentId)
      throws Exception {
  ...
  protected abstract void executeContentRequestAndValidateContent(String url, String content)
      throws Exception;

  protected abstract void verifyTwoDeleteEvents(
  ...
  protected abstract void verifySingleCreateAndUpdateEvent(
