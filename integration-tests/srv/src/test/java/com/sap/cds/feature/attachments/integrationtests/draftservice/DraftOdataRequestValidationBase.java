...
abstract class DraftOdataRequestValidationBase {
  ...
  void contentCanBeReadFromDraft() throws Exception {
    ...
    assertThat(selectedRoot.getItems().get(0).getAttachments())
        .hasSize(1)
        .first()
        .satisfies(attachment -> verifyContent(attachment.getContent(), testContentAttachment));
    assertThat(selectedRoot.getItems().get(0).getAttachmentEntities())
        .hasSize(1)
        .first()
        .satisfies(
    ...
    assertThat(attachmentResponse.getResponse().getContentAsString())
        .isEqualTo(testContentAttachment);
    var attachmentEntityResponse = requestHelper.executeGet(attachmentEntityUrl);
    assertThat(attachmentEntityResponse.getResponse().getContentAsString())
        .isEqualTo(testContentAttachmentEntity);
    verifyTwoReadEvents();
  }
  ...
  void deleteContentInDraft() throws Exception {
    ...
    var selectedRootAfterDelete = selectStoredRootData(selectedRoot);
    verifyContent(
        selectedRootAfterDelete.getItems().get(0).getAttachments().get(0).getContent(), null);
    verifyContent(
  ...
  void doNotDeleteContentInCancelledDraft() throws Exception {
    ...
    var selectedRootAfterDelete = selectStoredRootData(selectedRoot);
    verifyContent(
        selectedRootAfterDelete.getItems().get(0).getAttachments().get(0).getContent(),
  ...
  void updateContentInDraft() throws Exception {
    ...
    putNewContentForAttachment(
        newAttachmentContent, selectedRoot.getItems().get(0).getId(), itemAttachment.getId());
    ...
    var selectedRootAfterUpdate = selectStoredRootData(selectedRoot);
    verifyContent(
        selectedRootAfterUpdate.getItems().get(0).getAttachments().get(0).getContent(),
        newAttachmentContent);
    verifyContent(
        selectedRootAfterUpdate.getItems().get(0).getAttachmentEntities().get(0).getContent(),
  ...
  void noChangesOnAttachmentsContentStillAvailable() throws Exception {
    ...
    requestHelper.executePatchWithODataResponseAndAssertStatusOk(
        rootUrl, "{\"title\":\"some other title\"}");
  ...
  void deleteItemAndActivateDraft() throws Exception {
    ...
    verifyOnlyTwoDeleteEvents(
        selectedRoot.getItems().get(0).getAttachments().get(0).getContentId(),
  ...
  void deleteItemAndCancelDraft() throws Exception {
    ...
    assertThat(selectedRootAfterDelete.getItems().get(0).getAttachments().get(0).getContentId())
        .isNotEmpty();
  ...
  protected DraftRoots deepCreateAndActivate(
      String testContentAttachment, String testContentAttachmentEntity) throws Exception {
  ...
  private void createAttachmentWithContent(String testContentAttachment, String itemId)
      throws Exception {
    createAttachmentWithContent(testContentAttachment, itemId, status().isNoContent(), false);
  }

  private void createAttachmentWithContent(
      String testContentAttachment, String itemId, ResultMatcher matcher, boolean withError)
      throws Exception {
  ...
  private void putNewContentForAttachment(
      String testContentAttachment, String itemId, String attachmentId) throws Exception {
    putNewContentForAttachment(testContentAttachment, itemId, attachmentId, status().isNoContent());
  }

  private void putNewContentForAttachment(
      String testContentAttachment, String itemId, String attachmentId, ResultMatcher matcher)
      throws Exception {
  ...
  private void createAttachmentEntityWithContent(
      String testContentAttachmentEntity, Items responseItem) throws Exception {
    createAttachmentEntityWithContent(
        testContentAttachmentEntity, responseItem, status().isNoContent(), false);
  }

  private void createAttachmentEntityWithContent(
      String testContentAttachmentEntity,
      Items responseItem,
    ...
    putNewContentForAttachmentEntity(
        testContentAttachmentEntity, responseAttachmentEntity.getId(), matcher);
  }

  private void putNewContentForAttachmentEntity(
      String testContentAttachmentEntity, String attachmentId) throws Exception {
    putNewContentForAttachmentEntity(
  ...
  protected String getAttachmentBaseUrl(
      String itemId, String attachmentId, boolean isActiveEntity) {
    return BASE_URL
  ...
  protected void readAndValidateActiveContent(
    ...
    assertThat(attachmentEntityResponse.getResponse().getContentAsString())
        .isEqualTo(attachmentEntityContent);
    verifyTwoReadEvents();
  }
  ...
  private void updateFileName(
      DraftRoots selectedRoot,
      Attachments itemAttachment,
      AttachmentEntity itemAttachmentEntity,
  ...
  private void updateContentWithErrorAndValidateEvents(
      DraftRoots selectedRoot, Attachments itemAttachment, AttachmentEntity itemAttachmentEntity)
      throws Exception {
    testPersistenceHandler.setThrowExceptionOnUpdate(true);
    var newAttachmentContent = "new content attachment";
    putNewContentForAttachment(
        newAttachmentContent,
        selectedRoot.getItems().get(0).getId(),
        itemAttachment.getId(),
        status().is5xxServerError());
    var newAttachmentEntityContent = "new content attachmentEntity";
    putNewContentForAttachmentEntity(
        newAttachmentEntityContent, itemAttachmentEntity.getId(), status().is5xxServerError());
    verifyTwoCreateAndRevertedDeleteEvents();
    clearServiceHandlerContext();
  }

  private void verifyNothingHasChangedInDraft(
      DraftRoots selectedRoot, String attachmentContent, String attachmentEntityContent)
      throws IOException {
    var selectedRootAfterUpdate = selectStoredRootData(selectedRoot);
  ...
  private void createNewContentAndValidateEvents(DraftRoots selectedRoot) throws Exception {
    var itemAttachment = selectedRoot.getItems().get(0);
    var newAttachmentContent = "new attachment content";
    createAttachmentWithContent(
        newAttachmentContent, itemAttachment.getId(), status().is5xxServerError(), true);
    testPersistenceHandler.reset();
    var newAttachmentEntityContent = "new attachmentEntity content";
    createAttachmentEntityWithContent(
        newAttachmentEntityContent, itemAttachment, status().is5xxServerError(), true);
    verifyTwoCreateAndDeleteEvents(newAttachmentContent, newAttachmentEntityContent);
    clearServiceHandlerContext();
    testPersistenceHandler.reset();
  }

  private String getResponseContent(MvcResult attachmentResponse)
      throws UnsupportedEncodingException {
    return attachmentResponse.getResponse().getStatus() == HttpStatus.OK.value()
        ? attachmentResponse.getResponse().getContentAsString()
        : "";
  }

  protected abstract void verifyContentId(String contentId, String attachmentId);

  protected abstract void verifyContent(InputStream attachment, String testContent)
      throws IOException;
  ...
  protected abstract void verifyOnlyTwoCreateEvents(
      String newAttachmentContent, String newAttachmentEntityContent);

  protected abstract void verifyTwoCreateAndDeleteEvents(
      String newAttachmentContent, String newAttachmentEntityContent);

  protected abstract void verifyTwoReadEvents();

  protected abstract void verifyOnlyTwoDeleteEvents(
      String attachmentContentId, String attachmentEntityContentId);

  protected abstract void verifyTwoUpdateEvents(
